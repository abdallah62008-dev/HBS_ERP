...
| **Receive card** *(Phase 3)* | Visible **only** when `return_status === 'Pending'` AND `can('returns.receive')`. Single button — *"Mark received"*. Renders above the Inspect form. Disappears once the return moves out of Pending. | Pure lifecycle marker. The on-screen helper text spells out that inventory + refunds are untouched. |
| **Inspection section** | Visible inspection form when `return_status` is `Pending` or `Received` AND `can('returns.inspect')`. Once inspected, becomes a read-only summary (condition / restockable / inspector / timestamp). | This is the gate from Active to Resolved-ish (`Restocked`/`Damaged`). Phase 3 made Receive an optional checkpoint — operators can still inspect directly from Pending. |
