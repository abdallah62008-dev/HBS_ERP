        ...
        <AuthenticatedLayout header="Expenses">
            ...
            <form onSubmit={apply} className="mb-4 flex flex-wrap items-end gap-2 rounded-lg border border-slate-200 bg-white p-3">
                ...
                <select value={cashboxId} onChange={(e) => setCashboxId(e.target.value)} className="rounded-md border-slate-300 text-sm">
                    <option value="">Any cashbox</option>
                ...
                <table className="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {expenses.data.length === 0 && (
                            <tr><td colSpan={10} className="px-4 py-12 text-center text-sm text-slate-400">No expenses match.</td></tr>
                        )}
                        {expenses.data.map((e) => (
                            <tr key={e.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-4 py-2.5 text-xs text-slate-600">
                                    {e.payment_method ? <span>{e.payment_method.name}</span> : <span className="text-slate-400">—</span>}
                                </td>
                                <td className="px-4 py-2.5 text-xs text-slate-600">
                                    {e.cashbox ? <span>{e.cashbox.name}</span> : <span className="text-slate-400">—</span>}
                                </td>
                                <td className="px-4 py-2.5">
                                    {e.cashbox_transaction_id ? (
                                        <span className="inline-flex rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700">Posted</span>
                                    ) : (
                                        <span className="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-500">Unposted</span>
                                    )}
                                </td>
